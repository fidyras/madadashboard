...
```{r map setup, echo=FALSE, eval=TRUE,warning=FALSE,message=FALSE}
options(scipen = 10)
library(tidyverse)
library(sf)
library(stringdist)
...
**Prioritizing vaccination** {.sidebar}
-----------------------------------------------------------------------

\
\
In March 2021, Madagascar has initiated the necessary steps to re-join the COVAX initiative led by WHO...
When vaccines will arrive in Madagascar, they will probably be in limited supply and will require prioritization of available doses to achieve the best possible protection for the population.
Several lines of evidence and GAVI-COVAX rules show that priority should be given to healthcare workers and then to older adults.
Collectively these research indicate that vaccinating oldest individuals first saves the most lives and maximises years of remaining life expectancy and give further support to announced priority rankings.
\
\
Here we leverage data collected as part of this dashboard and use data collected by the INSTAT to potentially guide vaccination and vaccine education efforts in Madagascar based on a narrow but central aspect of vaccination prioritization: Distribution of age and cases in the Malagasy population.
...
### **Elderly population of Madagascar ** 
...
### **Mapping region with large number of older individuals ** 
...
library(leaflet)
bins <- pretty(mdg2$prop60)
pal <- colorBin("YlGnBu", domain = mdg2$prop60)
labels <- sprintf(
  "<strong>%s</strong><br/> Population of elderly: %i <br/>",
  mdg2$NAME,
  mdg2$n60,
  mdg2$prop60
) %>% lapply(htmltools::HTML)
map_old<-leaflet()  %>%
  ...
  addLegend(
    ...
    pal = pal,
    values = bins)
map_old %>% addProviderTiles(providers$Esri.WorldStreetMap)
...
### **Mapping region with most cases ** 
...
### **Regions with high case numbers and large population of Older individuals** 

```{r}

mdg2%>%
  mutate(age_cat=ifelse(n60>median(n60),2,1))%>%
  mutate(cases_cat=ifelse(cases>median(cases),2,1))%>%
  mutate(category=as.factor(cases_cat+age_cat))->mdg2



library(gghighlight)
my_breaks <- c(0,2,10,40,100,200)
ggplot(pop_age_Region,aes(x=cases,y=n60))+
  geom_point(aes(size=pop,color=category))+
  scale_y_continuous(trans="log10")+
  scale_x_continuous(trans="log10")+
  geom_text(aes(label=Region),hjust=0, vjust=0)+
  #gghighlight(cases>median(cases),label_key = NAME)+
  #scale_color_viridis_c(option="C",name="Number of deaths",trans="log10",breaks=my_breaks,labels=my_breaks)+
  scale_size(name="Population",range=c(1,8))+
...
panel.background = element_blank(), axis.line = element_line(colour = "black"))->fig1
