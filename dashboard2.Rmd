...
cases%>%
  arrange(Date)%>%
  select(Date,"cases"=n.cases,"deaths"=n.deaths,"tests"=n.tests,"total.tests"=Global.total.test)%>%
...
cases_region%>%
  group_by(region)%>%
  summarise(cases=sum(cases),deaths=sum(death,na.rm=TRUE))->region_cumulative #sum of cases by region since March
...
cases_region$region[is.na(cases_region$region)] = "unspecified"
cases_region%>% filter(region!="unspecified") %>% 
  rename("Date"=week)%>%
  arrange(Date)%>%
  group_by(region) %>%
  mutate(cases.cumsum = cumsum(cases),
...
valueBox(
  value = format(round(last(cases$cases.mov.avg),2), big.mark = ","),
  caption = paste("Average number of cases reported daily between",format(last(cases$Date)-7, format = "%B %d %Y"), " and ",
    format(last(cases$Date), format = "%B %d %Y")),
  icon = "fa-viruses",
  color = "warning"
  )
...
mindate<- last(sort(cases_region$Date))-15
recent <- cases_region %>% filter (Date>mindate) %>% group_by(region) %>% summarise (cas_recent=sum(cases))

mada_regions <-mdg2 %>% 
  left_join(region_cumulative, by = c("NAME" = "region")) %>%  
  left_join(recent,by = c("NAME" = "region"))
...
### `r f.tr("map_of_recent_cases_header")`
```{r}
bins_recent <- round(exp(pretty(log(recent$cas_recent), n = 10)), 0) # logged breaks
pal <- colorBin("YlOrRd", domain = mada_regions$cas_recent, bins = bins_recent)
labels <- sprintf(
  "<strong>%s</strong><br/> Cases: %i <br/>",
  mada_regions$NAME,
  mada_regions$cas_recent
) %>% lapply(htmltools::HTML)
map_recent <- leaflet()  %>%
  addTiles() %>%
  addPolygons(
...
map_recent %>% addProviderTiles(providers$Esri.WorldStreetMap)
