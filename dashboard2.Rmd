...
pal <- colorBin("YlOrRd", domain = mada_regions$cases,bins=6)
labels <- sprintf(
...
```
On `r date_updated`, the average of R_t over the past 7 days is `r round(last(r_mdg$reff),2)` for Madagascar.

Methods {.sidebar}
-----------------------------------------------------------------------
The time varying reproductive number (R_t), which represents the average number of secondary infections generated by each new infectious case, is a way to track the progress of an outbreak. If R_t in the population is greater than 1, new cases will continue to increase. If R_t is less than 1, new cases will decrease. The higher the value of R_t above 1, the faster an epidemic will grow. 
\
R_t was estimated using the method described here and here.

This estimation framework accounts for:
- delay from case onset to case confirmation
- delay from infection to case onset (i.e. the incubation period)
- uncertainty in the serial interval (Cori)
- choice of smoothing window

This estimation frameowrk does not account for:
- changes in testing across time
- differences in testing across space

Representative and robust testing are needed to make these estimates more accurate and informative of outbreak paths. 

See here for a review of considerations and uncertainties associated with estimating Rt.

These estimates were last updated on date.
...
gauge(round(last(r_mdg$reff),2),min=0,max=5,symbol='',
      label=paste("Reff on: ",format(last(r_mdg$Date), format="%B %d %Y")),abbreviateDecimals = 2,
...
```{r}
diff_r<-last(r_mdg$reff)-nth(r_mdg$reff,length(r_mdg$reff)-7)
txt<-ifelse(diff_r<0,"Decreasing","Increasing")
valueBox(paste (txt,"[",round(diff_r,2),"]"), 
         caption="Trend of Re compared to 1 week ago",
         color=ifelse(diff_r<0,"success","danger"))
```
...
### Estimating R effective regionally


```{r}
gauge(round(last(r_tmm$reff),2),min=0,max=5,symbol='',
      label=paste("Reff in Atsinanana on: ",format(last(r_tmm$Date), format="%B %d %Y")),abbreviateDecimals = 2,
      sectors=gaugeSectors(success=c(0,1),warning = c(1,2),danger=c(2,5)))
```
...
```{r}
diff_r<-last(r_tmm$reff)-nth(r_tmm$reff,length(r_tmm$reff)-7)
txt<-ifelse(diff_r<0,"Decreasing","Increasing")
valueBox(paste (txt,"[",round(diff_r,2),"]"), 
         caption="Trend of Re compared to 1 week ago",
         color=ifelse(diff_r<0,"success","danger"))
...
gauge(round(last(r_tnr$reff),2),min=0,max=5,symbol='',
      label=paste("Reff of Analamanga on: ",format(last(r_tnr$Date), format="%B %d %Y")),abbreviateDecimals = 2,
      sectors=gaugeSectors(success=c(0,1),warning = c(1,2),danger=c(2,5)))
```
...
```{r}
diff_r<-last(r_tnr$reff)-nth(r_tnr$reff,length(r_tnr$reff)-7)
txt<-ifelse(diff_r<0,"Decreasing","Increasing")
valueBox(paste (txt,"[",round(diff_r,2),"]"), 
         caption="Trend of Re compared to 1 week ago",
         color=ifelse(diff_r<0,"success","danger"))
