...
bins <- round(exp(pretty(log(cases_by_region$cases + 1e-6), n = 6)), -1) # logged breaks
...
sd %>%
  ggplot(aes(
    ...
    text = map(paste("<b>", region, "</b><br>",
                     ...
                     "Moving avg:", round(mov.avg, 2), "<br>"), HTML))
  ) +
  geom_col(
...
bscols(
  (ggplotly(modavg.curv, tooltip = "text") %>% 
...
bscols(
  (ggplotly(cumulative_cases, tooltip = c("text")) %>%
    layout(yaxis = list(autorange = TRUE)) %>%
...
ntl.cases %>% select(c(Date, "positivity" = positivity, "Moving.average" =
  ...
  ggplot(aes(x = Date, 
             text =  map(paste(f.tr("daily_pos_xlab"), Date, "<br>",
                               ...
                               "Moving average", ":", round(Moving.average, 2), "<br>"), HTML))) +
...
\
\
R~t~ was estimated using the method described [here](https://epiforecasts.io/covid/), using the R packages [`epiNow`](https://epiforecasts.io/EpiNow/), [`epiSoon`](https://github.com/epiforecasts/EpiSoon), and [`epiEstim`](), code available [here](https://cran.r-project.org/web/packages/EpiEstim/index.html).
\
\
...
```{r}
# Either forecast for today or the latest date available
if(max(rt_ests$date) < Sys.Date()) rt_date <- max(rt_ests$date) else rt_date <- Sys.Date()

rt_ests %>%
  filter(region == "National", 
         date == rt_date) %>% .$median -> rt_national

gauge(round(rt_national, 2), min = 0, max = 5, symbol = '',
      label = paste("Estimate on", format(rt_date, format="%B %d %Y")),
      abbreviateDecimals = 2,
