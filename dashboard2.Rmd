...
r_mdg %>%
  ...
  ylab("R effective") +
  ggtitle("Effective Reproductive number of COVID19 estimates \n in Madagascar")->Re_mdg
...
gauge(round(last(r_tmm$reff),2),min=0,max=5,symbol='',
      label=paste("Reff on: ",format(last(r_tmm$Date), format="%B %d %Y")),abbreviateDecimals = 2,
      sectors=gaugeSectors(success=c(0,1),warning = c(1,2),danger=c(2,5)))
```
...
gauge(round(last(r_tnr$reff),2),min=0,max=5,symbol='',
      label=paste("Reff on: ",format(last(r_tnr$Date), format="%B %d %Y")),abbreviateDecimals = 2,
      sectors=gaugeSectors(success=c(0,1),warning = c(1,2),danger=c(2,5)))
```
...
r_tnr %>%
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+theme_bw()+
  ylab("R effective") +
  ggtitle("Effective Reproductive number of COVID19 estimates \n in Atsinanana region, Madagascar")->Re_tnr
ggplotly(Re_tnr)%>%
  layout(hovermode="x")
```