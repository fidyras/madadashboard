...
cases_region%>%group_by(region)%>%summarise(cases=sum(cases))->region_cumulative #sum of cases by region since March
...
date_updated <- format(last(cases$week), format = "%B %d %Y") # any inline stats you need ahead of time!
```
...
Through `r last(cases$week)`, Madagascar has officially registered `r format(last(cases$cum.cases),scientific=FALSE)` cases and `r format(last(cases$cum.deaths),scientific=FALSE)` deaths.
