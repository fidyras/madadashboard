...
```{r cfr}
weekly<-weekly%>%
  mutate(cum.deaths=cumsum(n.deaths),cfr=cum.deaths/cum.cases*100)
...
a%>%
  ...
  group_by(region)%>%
  mutate(cum.deaths=cumsum(death),cum.cases=cumsum(cases),cfr=cum.deaths/cum.cases*100)->a

ggplot(a)+
  geom_line(aes(x=week,y=cfr,color=region,group=region),alpha=0.2)+
  geom_smooth(data=weekly,aes(x = week, y = cfr),method="gam",se=FALSE) +
