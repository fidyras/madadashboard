...
weekly_region$week<-as.Date(weekly_region$week)
weekly_region%>%group_by(region)%>%summarise(cases=sum(cases))->region_cumulative #sum of cases by region since March
weekly<-weekly%>%arrange(week)%>%mutate(cum.cases=cumsum(n.cases))
...
date_updated <- format(last(weekly$week), format = "%B %d %Y") # any inline stats you need ahead of time!
```
...
```{r}
valueBox(
  value = format(sum(weekly$n.cases,na.rm=TRUE), big.mark = ","),
  caption = paste("total number of cases until",
                  format(last(weekly$week), format = "%B %d %Y")),
  icon = "fa-ambulance",
  color = "info"
)
```
...
valueBox(
  value = sum(weekly$n.deaths, na.rm = TRUE),
...
valueBox(
  value = format(last(weekly$n.cases), big.mark = ","),
...
valueBox(
  value = format(last(weekly$n.tests), big.mark = ","),
  caption = paste("Number of tests performed on",
    format(last(weekly$week), format = "%B %d %Y")),
  icon = "fa-flask",
  color = ifelse(last(weekly$n.tests) == min(weekly$n.tests), "warning", "info")
  )
...
weekly %>% filter(n.tests!=0)%>%mutate(positivity=n.cases/n.tests*100)->weekly.pos
gauge(
  round(last(weekly.pos$positivity), 2),
  min = 0,
  max = 100,
  symbol = '%',
  label = paste("positivity during week \n ending on",
    last(weekly$week)),
...
valueBox(
  value = paste(round(100 * sum(weekly$n.deaths) / sum(weekly$n.cases), 2), "%"),
...
weekly.cases <- ggplot(data = weekly, aes(x = week, y = n.cases)) +
  geom_col(aes(x = week, y = n.cases, fill = frequence)) +
  scale_x_date(date_breaks = "2 weeks", date_labels = "%Y-%m-%d") +
  scale_fill_manual(values=c("#7f2704","#fdd0a2"), name="Legende",labels=c("cas par semaine","cas par jour")) +
  theme( 
        panel.background = element_blank(),
        axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = "Date",
       y = "Number of cases")
ggplotly(weekly.cases) 
```
...
bins <- round(exp(pretty(log(cases_by_region$cases), n = 10)), 0) # logged breaks
...
weekly_region %>%
  group_by(region) %>%
  mutate(cum.sum=cumsum(cases),max_sum = max(cum.sum)) %>%
  ungroup() %>% 
...
sd %>%
  ggplot(aes(x = week, y = cum.sum, fill = region, 
           text = map(paste("<b>", region, "</b><br>",
                       f.tr("time_series_b_xlab"), ":", week, "<br>",
...
sd%>%
  ggplot() +
  geom_col(
    aes(x =week, y = cases, fill = region),
    position = "dodge",
    alpha = 0.2,
    size = 0.1
  ) +
...
```{r, warning=FALSE}
weekly.deaths <- ggplot(data = weekly, aes(x = week, y = n.deaths)) +
  geom_col(aes(x = week, y = n.deaths, fill = frequence)) +
    scale_x_date(date_breaks = "2 weeks") +
    scale_fill_manual(values=c("#67000d","#fcbba1"), name="Legende",labels=c(hebdomadaire="deces par semaine",quotidien="deces par jour"))  +
  theme( 
        panel.background = element_blank(),
        axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = "Week ending on",
       y = "Number of deaths")
ggplotly(weekly.deaths) %>% 
  layout(hovermode = "x") %>%
  config(displaylogo = FALSE)
...
Through `r last(weekly$week)`, Madagascar has officially registered `r format(last(weekly$cum.cases),scientific=FALSE)` cases and `r format(last(weekly$cum.deaths),scientific=FALSE)` deaths.
...
```{r cfr}
weekly<-weekly%>%
  mutate(cum.deaths=cumsum(n.deaths),cfr=round(cum.deaths/cum.cases*100,2))

deaths%>%
  mutate(week=lubridate::ceiling_date(Date,unit="week"))%>%
  filter(Type=="D")%>%
  group_by(week,Location4)%>%
  summarise(death=n())%>%
  filter(!is.na(Location4))%>%rename("region"=Location4)->d_region_week #using data from March-October, compute weekly number of deaths recorded per region

weekly_region%>%
  filter(!is.na(death))%>%
  select(week,region,death)%>%
  rbind(d_region_week)->d_region_week #

weekly_region%>%
  select(week,region,cases)%>%
  left_join(d_region_week)->a #join number of deaths and number of cases per region per week since March

a$death<-ifelse(is.na(a$death),0,a$death)

a%>%
  filter(!is.na(region),region!="Melaky")%>%
  group_by(region)%>%
  mutate(cum.deaths=cumsum(death),cum.cases=cumsum(cases),cfr=round(cum.deaths/cum.cases,2)*100)->a

ggplot(a)+
  geom_line(aes(x=week,y=cfr,color=region,group=region),alpha=0.2)+
  geom_line(data=weekly,aes(x = week, y = cfr),method="gam",se=FALSE) +
...
weekly%>%
  ggplot(aes(x = week)) +
  geom_col(aes(y = n.tests, fill = frequence),
           size = 0.5,
           alpha = 0.7) +
  scale_x_date(date_breaks = "2 weeks") +
  scale_y_continuous(breaks=seq(0,7000,1000))+
  scale_fill_manual(values=c("#3f007d","#bcbddc"), name="Legende",labels=c(hebdomadaire="tests par semaine",quotidien="tests par jour")) +
  labs(x = "Week ending on",
       y = "Number of tests")+
  theme(
    panel.background = element_blank(),
