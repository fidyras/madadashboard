...
bins <- c(0,50,100,500,1000,1500,3000,6000,12000)
...
modavg.curv<-ggplot(data=cases_by_date, aes(x=Date,y=mov.avg,color=region,fill=region))+
  ...
  theme(panel.background=element_blank(), axis.text.x = element_text(angle = 45, hjust = 1))
ggplotly(modavg.curv)%>%config(displaylogo=FALSE)
```

### Analamanga

```{r, warning=FALSE}
cases_by_date%>%filter(region=="Analamanga")%>%
ggplot(aes(x=Date,y=mov.avg,color=region,fill=region))+
  geom_col(aes(x=Date,y=cases,fill=region),position="dodge",alpha=0.2,size=0.1)+
  scale_x_date(date_breaks = "2 weeks") +
  geom_line(aes(x=Date,y=mov.avg,colour=region),size=0.7,method="auto")+
  labs(x=f.tr("time_series_d_xlab"), y=f.tr("time_series_d_ylab")) +
  theme(panel.background=element_blank(), axis.text.x = element_text(angle = 45, hjust = 1))->modavg_alg
ggplotly(modavg_alg)%>%config(displaylogo=FALSE)
```

### Atsinanana

```{r, warning=FALSE}
cases_by_date%>%filter(region=="Atsinanana")%>%
ggplot(aes(x=Date,y=mov.avg,color=region,fill=region))+
  geom_col(aes(x=Date,y=cases,fill=region),position="dodge",alpha=0.2,size=0.1)+
  scale_x_date(date_breaks = "2 weeks") +
  geom_line(aes(x=Date,y=mov.avg,colour=region),size=0.7,method="auto")+
  labs(x=f.tr("time_series_d_xlab"), y=f.tr("time_series_d_ylab")) +
  theme(panel.background=element_blank(), axis.text.x = element_text(angle = 45, hjust = 1))->modavg_ats
ggplotly(modavg_ats)%>%config(displaylogo=FALSE)
```

### Alaotra-Mangoro

```{r, warning=FALSE}
cases_by_date%>%filter(region=="Alaotra-Mangoro")%>%
ggplot(aes(x=Date,y=mov.avg,color=region,fill=region))+
  geom_col(aes(x=Date,y=cases,fill=region),position="dodge",alpha=0.2,size=0.1)+
  scale_x_date(date_breaks = "2 weeks") +
  geom_line(aes(x=Date,y=mov.avg,colour=region),size=0.7,method="auto")+
  labs(x=f.tr("time_series_d_xlab"), y=f.tr("time_series_d_ylab")) +
  theme(panel.background=element_blank(), axis.text.x = element_text(angle = 45, hjust = 1))->modavg_alM
ggplotly(modavg_alM)%>%config(displaylogo=FALSE)
```
...
```{r, warning=FALSE}
cases_by_date%>%filter(region=="Analanjirofo")%>%
ggplot(aes(x=Date,y=mov.avg,color=region,fill=region))+
  geom_col(aes(x=Date,y=cases,fill=region),position="dodge",alpha=0.2,size=0.1)+
  scale_x_date(date_breaks = "2 weeks") +
  geom_line(aes(x=Date,y=mov.avg,colour=region),size=0.7,method="auto")+
  labs(x=f.tr("time_series_d_xlab"), y=f.tr("time_series_d_ylab")) +
  theme(panel.background=element_blank(), axis.text.x = element_text(angle = 45, hjust = 1))->modavg_alj
ggplotly(modavg_alj)%>%config(displaylogo=FALSE)
```
...
```{r, warning=FALSE}
cases_by_date%>%filter(region=="Diana")%>%
ggplot(aes(x=Date,y=mov.avg,color=region,fill=region))+
  geom_col(aes(x=Date,y=cases,fill=region),position="dodge",alpha=0.2,size=0.1)+
  scale_x_date(date_breaks = "2 weeks") +
  geom_line(aes(x=Date,y=mov.avg,colour=region),size=0.7,method="auto")+
  labs(x=f.tr("time_series_d_xlab"), y=f.tr("time_series_d_ylab")) +
  theme(panel.background=element_blank(), axis.text.x = element_text(angle = 45, hjust = 1))->modavg_dia
ggplotly(modavg_dia)%>%config(displaylogo=FALSE)
```

### Anosy

```{r, warning=FALSE}
cases_by_date%>%filter(region=="Anosy")%>%
ggplot(aes(x=Date,y=mov.avg,color=region,fill=region))+
  geom_col(aes(x=Date,y=cases,fill=region),position="dodge",alpha=0.2,size=0.1)+
  scale_x_date(date_breaks = "2 weeks") +
  geom_line(aes(x=Date,y=mov.avg,colour=region),size=0.7,method="auto")+
  labs(x=f.tr("time_series_d_xlab"), y=f.tr("time_series_d_ylab")) +
  theme(panel.background=element_blank(), axis.text.x = element_text(angle = 45, hjust = 1))->modavg_anosy
ggplotly(modavg_anosy)%>%config(displaylogo=FALSE)
```

### Vakinankaratra

```{r, warning=FALSE}
cases_by_date%>%filter(region=="Vakinankaratra")%>%
ggplot(aes(x=Date,y=mov.avg,color=region,fill=region))+
  geom_col(aes(x=Date,y=cases,fill=region),position="dodge",alpha=0.2,size=0.1)+
  scale_x_date(date_breaks = "2 weeks") +
  geom_line(aes(x=Date,y=mov.avg,colour=region),size=0.7,method="auto")+
  labs(x=f.tr("time_series_d_xlab"), y=f.tr("time_series_d_ylab")) +
  theme(panel.background=element_blank(), axis.text.x = element_text(angle = 45, hjust = 1))->modavg_vak
ggplotly(modavg_vak)%>%config(displaylogo=FALSE)
```

### Atsimo-Andrefana

```{r, warning=FALSE}
cases_by_date%>%filter(region=="Atsimo-Andrefana")%>%
ggplot(aes(x=Date,y=mov.avg,color=region,fill=region))+
  geom_col(aes(x=Date,y=cases,fill=region),position="dodge",alpha=0.2,size=0.1)+
  scale_x_date(date_breaks = "2 weeks") +
  geom_line(aes(x=Date,y=mov.avg,colour=region),size=0.7,method="auto")+
  labs(x=f.tr("time_series_d_xlab"), y=f.tr("time_series_d_ylab")) +
  theme(panel.background=element_blank(), axis.text.x = element_text(angle = 45, hjust = 1))->modavg_tol
ggplotly(modavg_tol)%>%config(displaylogo=FALSE)
```

### Boeny

```{r, warning=FALSE}
cases_by_date%>%filter(region=="Boeny")%>%
ggplot(aes(x=Date,y=mov.avg,color=region,fill=region))+
  geom_col(aes(x=Date,y=cases,fill=region),position="dodge",alpha=0.2,size=0.1)+
  scale_x_date(date_breaks = "2 weeks") +
  geom_line(aes(x=Date,y=mov.avg,colour=region),size=0.7,method="auto")+
  labs(x=f.tr("time_series_d_xlab"), y=f.tr("time_series_d_ylab")) +
  theme(panel.background=element_blank(), axis.text.x = element_text(angle = 45, hjust = 1))->modavg_mja
ggplotly(modavg_mja)%>%config(displaylogo=FALSE)
```
...
R estimate
===================================
### Estimating R effective for Madagascar

```{r}
ntl.cases%>%
  select(Date,"cases"=cum.cases) %>%
  mutate(Confirmed = cases) %>%
  select(Date,Confirmed)%>%
  mutate(epiweek = epiweek(Date))->covid_pt
...
covid_mdg$Confirmed_var[ covid_mdg$Confirmed_var < 0 ] <- 0

## parametric estimate given 'known' SI (no CIs)
res_parametric_si <-
  estimate_R(
    covid_mdg$Confirmed_var,
...
r_mdg <- as.data.frame(res_parametric_si$R)
...
####estimating Re for Atsinanana region
cases_by_date%>%
  select(Date,"cases"=cum.sum) %>%
  filter(region=="Atsinanana")%>%
  mutate(Confirmed = cases) %>%
  select(Date,Confirmed) -> covid_pt_tmm


covid_pt_tmm<-covid_pt_tmm  %>%
  #subset(Date >= '2020-03-05') %>%
  mutate(epiweek = epiweek(Date))

first.date <- head(covid_pt_tmm$Date,1)
...
covid_pt_tmm %>%
  ...
  subset(Date >  first.date) -> covid_pt_tmm
...
## set negative daily counts to zero
covid_tmm$Confirmed_var[ covid_tmm$Confirmed_var < 0 ] <- 0

## parametric estimate given 'known' SI (no CIs)
res_parametric_si <-
  estimate_R(
    covid_tmm$Confirmed_var,
    method ="parametric_si",
    config = make_config(
      list(
        mean_si = 4.7,
...
r_tmm <- as.data.frame(res_parametric_si$R)
...
####estimating Re for Analamanga region
cases_by_date%>%
  select(Date,"cases"=cum.sum) %>%
  filter(region=="Analamanga")%>%
  mutate(Confirmed = cases) %>%
  select(Date,Confirmed) -> covid_pt_tnr


covid_pt_tnr<-covid_pt_tnr  %>%
  #subset(Date >= '2020-03-05') %>%
  mutate(epiweek = epiweek(Date))

first.date <- head(covid_pt_tnr$Date,1)
...
covid_pt_tnr %>%
  ...
  subset(Date >  first.date) -> covid_pt_tnr
...
## set negative daily counts to zero
covid_tnr$Confirmed_var[ covid_tnr$Confirmed_var < 0 ] <- 0

## parametric estimate given 'known' SI (no CIs)
res_parametric_si <-
  estimate_R(
    covid_tnr$Confirmed_var,
    method ="parametric_si",
    config = make_config(
      list(
        mean_si = 4.7,
...
r_tnr <- as.data.frame(res_parametric_si$R)
...
The reproductive number Re (sometimes called Rt) of a disease is defined as the number of secondary cases that a single positive individual is expected to cause. In other words, it is the number of people who will get infected by one person on average. The higher the value of Re, the faster an epidemic will progress.
\
If the average Re in the population is greater than 1, the infection will spread exponentially. If Re is less than 1, the infection will spread only slowly, and it will eventually die out. The higher the value of Re, the faster an epidemic will progress. 
\
\
On `r date_updated`, the average of Re over the past 7 days is `r round(last(r_mdg$reff),2)` for Madagascar.
\

Sidebar {.sidebar}
-----------------------------------------------------------------------
The estimates of the Reproductive number shown here is dependent upon the testing capacity and results provided by the Malagasy government Taskforce. It does not attempt to correct for testing volume.
...
### Re estimate in Madagascar

```{r}
gauge(round(last(r_mdg$reff),2),min=0,max=5,symbol='',
      label=paste("National estimate of Rt \n on: ",format(last(r_mdg$Date), format="%B %d %Y")),abbreviateDecimals = 2,
...
```{r}
diff_r<-last(r_mdg$reff)
txt<-ifelse(diff_r<1,"Decreasing","Increasing")
valueBox(value= txt, 
         caption="Expected trend of the daily number of cases",
         color=ifelse(diff_r<1,"success","danger"))
```
...
```{r}
txt<-round((log(2)*5)/(last(r_mdg$reff)-1),0)
valueBox(paste(txt,"days"), 
         caption="Expected doubling time of cases",
         color=ifelse(last(r_tnr$reff)<1,"success","danger"))
```
...
### Time varying reproduction number estimates
...
r_mdg %>%
  ...
  geom_ribbon(aes(Date, ymin = r_low, ymax = r_high), alpha=0.3,fill="#33a02c") +
  scale_x_date(breaks='7 days') +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+theme_bw()+
  ylab("R effective") +
  ggtitle("Effective Reproductive number of COVID19 estimates in Madagascar")->Re_mdg
ggplotly(Re_mdg,dynamicTicks=TRUE)%>%
  layout(hovermode="x")
...
```{r}
gauge(round(last(r_tmm$reff),2),min=0,max=5,symbol='',
      label=paste("Rt in Atsinanana \n on: ",format(last(r_tmm$Date), format="%B %d %Y")),abbreviateDecimals = 2,
      sectors=gaugeSectors(success=c(0,1),warning = c(1,2),danger=c(2,5)))
...
```{r}
diff_r<-last(r_tmm$reff)
txt<-ifelse(diff_r<1,"Decreasing","Increasing")
valueBox(txt, 
         caption="Expected trend of daily number of cases",
         color=ifelse(diff_r<1,"success","danger"))
```
...
```{r}
txt<-round((log(2)*5)/(last(r_tmm$reff)-1),0)
valueBox(paste(txt,"days"), 
         caption="Expected doubling time of cases",
         color=ifelse(last(r_tnr$reff)<1,"success","danger"))
...
r_tmm %>%
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+theme_bw()+
  ylab("R effective") +
  ggtitle("Effective Reproductive number of COVID19 estimates in Atsinanana region, Madagascar")->Re_tmm
ggplotly(Re_tmm,dynamicTicks=TRUE)%>%
  #rangeslider()%>%
  layout(hovermode="x")
```
...
```{r}
gauge(round(last(r_tnr$reff),2),min=0,max=5,symbol='',
      label=paste("Rt of Analamanga \n on :",format(last(r_tnr$Date), format="%Y-%m-%d %Y")),abbreviateDecimals = 2,
      sectors=gaugeSectors(success=c(0,1),warning = c(1,2),danger=c(2,5)))
```
...
```{r}
diff_r<-last(r_tnr$reff)
txt<-ifelse(last(r_tnr$reff)<1,"Decreasing","Increasing")
valueBox(txt, 
         caption="Expected trend of daily number of cases",
         color=ifelse(diff_r<1,"success","danger"))
```
...
```{r}
txt<-round((log(2)*5)/(last(r_tnr$reff)-1),0)
valueBox(paste(txt,"days"), 
         caption="Expected doubling time of cases",
         color=ifelse(last(r_tnr$reff)<1,"success","danger"))
...
### Time Varying reproductive rate in Analamanga
...
r_tnr %>%
  ...
  geom_hline( yintercept=1) +
  geom_ribbon(aes(Date, ymin = r_low, ymax = r_high),alpha=0.3,fill="#33a02c") +
  scale_x_date(breaks='14 days') +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+theme_bw()+
  ylab("R effective") +
  ggtitle("Effective Reproductive number of COVID19 estimates in Analamanga region, Madagascar")->Re_tnr
ggplotly(Re_tnr)%>%
  layout(hovermode="x")
```
