---
title: "Plateforme d'informations Veterinaire | Madagascar"
output: 
...
weekly<-weekly%>%mutate(cum.cases=cumsum(n.cases))
...
valueBox(
  ...
  caption = paste("Nombre total de diarhee hemorragique",
...
valueBox(
  ...
  caption = paste("Nombre de cas de Babesiose confirmes par PCR",
                  format(last(weekly$week), format = "%B %d %Y")),
  color = "danger",
  icon = "fa-heartbeat"
)
...
valueBox(
  ...
  caption = paste("Nombre de cas presentant une hematurie",
    format(last(weekly$week), format = "%B %d %Y")),
  icon = "fa-viruses",
  color = "warning"
  )
...
valueBox(
  ...
  caption = paste("Nombre de consultations rapportees",
    format(last(weekly$week), format = "%B %d %Y")),
  icon = "fa-flask",
...
gauge(
  ...
  max = 100,
  symbol = '%',
  label = paste("Prevalence de Parvovirose chez les chiens",
    last(weekly$week)),
...
weekly.cases <- ggplot(data = weekly, aes(x = week, y = n.cases)) +
  geom_col(aes(x = week, y = n.cases, fill = "n.cases")) +
...
ggplotly(weekly.cases, tooltip = "text") %>% 
  layout(hovermode = "x") %>%
...
weekly.deaths <- ggplot(data = weekly, aes(x = week, y = n.deaths)) +
  geom_col(aes(x = week, y = n.deaths, fill = "n.deaths")) +
...
ggplotly(weekly.deaths, tooltip = "text") %>% 
  layout(hovermode = "x") %>%
  config(displaylogo = FALSE)
...
```{r}
weekly<-weekly%>%mutate(cum.deaths=cumsum(n.deaths),cfr=cum.deaths/cum.cases*100)
cfr_weekly<-weekly%>%
  ggplot(aes(x = week)) +
  geom_line(aes(x = week, y = cfr)) +
  scale_y_continuous(name="case fatality rate (%)", breaks=seq(0,5,0.5))+
  scale_x_date(date_breaks = "2 weeks") +
  scale_fill_manual(
    name = "",
    values = "#ff6262",
...
Using data on deaths that occurred and the number of cases, we can estimate the **case fatality ratio (CFR)**. The CFR is defined as the proportion of **confirmed Parvoviosis deaths** over the number of **confirmed Parvovirosis cases**.

The CFR can help us understand the severity of the disease in confirmed cases however it is *not the same as* the risk of death for an infected person and thus **does not** represent the probability of dying if an individual is infected by Parvovirosis.
...
weekly%>%
  ggplot(aes(x = week)) +
  geom_col(aes(y = n.tests, fill = "tests"),
           size = 0.5,
           alpha = 0.7) +
  scale_x_date(date_breaks = "2 weeks") +
  ...
  scale_fill_manual(
    name = "",
    values = c("tests" = "#8d66a1"),
    labels = "tests"
  ) +
  labs(x = "Week ending on",
       y = "Number of tests per week")+
  theme(
